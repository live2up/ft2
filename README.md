...
ft2 是一个纯 Python 量化研究框架，核心理念是**去黑箱**——控制每一层、理解每一行。从信号生成到回测执行到报告输出，所有环节自主实现，无第三方回测框架依赖。

**Fast** — 向量化回测 38ms/次，快速扫描策略空间；**Test** — 严谨的 fast/full 双模式验证，SR 偏差 < 0.000001；**Trade** — 从信号到策略的研究闭环，状态机择时 / 截面因子轮动 / 多风格组合，为交易研究而生。
