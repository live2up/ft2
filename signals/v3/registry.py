...
管理已发现的 Expression 实例，提供 CRUD 和查询能力。
...
============================================================================
 ...
 │  Expression 注册流程                                                  │
 │                                                                      │
 │  register(name, expr, category)  →  计算 fingerprint (SHA256)       │
 │                                  →  检查去重                        │
 │                                  →  存入 _entries 字典               │
 │                                                                      │
 │  指纹计算: SHA256(complexity + features_used 排序)                   │
 │  去重规则: 相同 complexity + 相同特征集 → 视为重复表达式              │
 └─────────────────────────────────────────────────────────────────────┘
...
============================================================================
 ...
 ┌─────────────────┬──────────────────────────────────────────────┐
 │  list_all()      │ 列出所有已注册表达式                          │
 │  list_by_category│ 按类别筛选 (volatility/trend/momentum/...)   │
 │  get(name)       │ 按名称精确查找                                │
 │  search(query)   │ 模糊搜索 (名称/类别/表达式字符串)             │
 │  top(n, metric)  │ 按绩效排序 (Sharpe/年化/回撤)                 │
 └─────────────────┴──────────────────────────────────────────────┘
