...
# 数学 (8个)
abs(x)  log(x)  sqrt(x)  sign(x)  exp(x)  tanh(x)  sigmoid(x)  relu(x)
...
- 63 原语覆盖 WorldQuant 时序算子的 96%，截面 100%
- 扩展新原语只需 3 步：写函数 → 注册 → LLM 即用
