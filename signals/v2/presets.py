...
内置预配置的参数化表达式模板，替代旧的 SIGNAL_TEMPLATES。
...
============================================================================
 ...
 │  模板语法：占位符机制                                                │
 │                                                                      │
 │  'ATR(?)'  ← ? 表示可参数化的占位符，类似 SQL prepared statement      │
 │                                                                      │
 │  ExpressionPreset("atr", "thr_mean(ATR(?))", ...)                   │
 │     ↓ 填充参数                                                       │
 │  from_preset('atr', param_grid={'?': [7, 14]})                       │
 │     ↓                                                                │
 │  "thr_mean(ATR(7))"                                                 │
 │  "thr_mean(ATR(14))"                                                │
 └─────────────────────────────────────────────────────────────────────┘
...
============================================================================
 ...
 │   类别        │   模板示例                                        │
 ├──────────────┼──────────────────────────────────────────────────┤
 │ volatility    │ thr_mean(ATR(?)), thr_mean(BBWIDTH(?))           │
 │ trend         │ thr_mean(EMA(?)), thr_mean(TRIMA(?))             │
 │ momentum      │ thr_mean(RSI(?)), thr_mean(CCI(?))               │
 │ statistic     │ thr_mean(TSF(?)), thr_mean(VAR(?))               │
 │ volume        │ thr_mean(VOL_RATIO(?)), thr_mean(OBV(?))         │
 │ composite     │ thr_mean(ATR(?)) & thr_mean(TRIMA(?))            │
 │ gp_seeds      │ GP 发现的优质种子表达式 (从 AIdev V5 导入)       │
 └──────────────┴──────────────────────────────────────────────────┘
...
============================================================================
 ...
 ┌───────────────┬────────────────────────────────────────────────┐
 │  name          │ 模板名称 (唯一标识)                             │
 │  template      │ 表达式字符串 (含 ? 占位符)                      │
 │  description   │ 功能描述                                       │
 │  category      │ 类别标签 (volatility/trend/momentum/...)       │
 │  param_count   │ 占位符数量                                     │
 │  example       │ 填充示例                                       │
 └───────────────┴────────────────────────────────────────────────┘
