...
对任意信号的择时表现进行结构化解释：
- 市场状态分割：波动率(高/中/低) × 趋势(强/弱) × 成交量(放量/缩量)
- 解释信号在每个状态下是否有效
- 生成人类可读的解释文本 + 可操作的改进建议

用法:
    from signals.v2 import Explainer, FeatureSpace
    fs = FeatureSpace()
    explainer = Explainer(fs)
    report = explainer.explain("RSI(14)", df)
    print(report.text)
"""
