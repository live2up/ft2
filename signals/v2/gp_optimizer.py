...
基于 AIdev GP V5 的成熟架构，移植自 d:\01-Doc\AIdev\002zeshi_gp_v5\gp_optimizer_v5.py。
核心能力：
1. 随机树生成（grow/full方法）
2. 子树交叉 + 子树变异 + 随机注入
3. 回测驱动适应度（train_sharpe × 过拟合惩罚 - 复杂度惩罚）
4. 适应度共享维护多样性
5. 精英保留 + 随机注入维持探索
6. 种子知识注入（从 PRESETS 和 SEED_EXPRESSIONS）
7. Train/Test 划分防止过拟合
...
技术路线: 当前 v2 采用两条互补路线
  ...
  │              │  目标: 拟合未来收益率 → IC/回测验证    │
  ├──────────────┼──────────────────────────────────────┤
  │  路线B: 自定义│  本文件 gp_optimizer.py 即为其基础     │
  │  SR 引擎     │  复用现有种群/算子/选择/共享框架       │
  │              │  仅需修改：                            │
  │              │    1. _evaluate_individual() 适应度     │
  │              │       从 Sharpe 改为 MSE 或 IC         │
  │              │    2. 目标变量从交易信号改为收益率      │
  │              │    3. 可选: 增加 SR 特有阈值函数节点    │
  └──────────────┴──────────────────────────────────────┘

用法:
    from signals.v2 import FeatureSpace, GPOptimizer
    ...
    gp = GPOptimizer(fs, train_data, test_data,
...
注意：
- 直接使用 v2 的 TreeNode / Expression / NodeType，不再重复定义
- 回测通过 v2 的 run_backtest / run_backtest_from_signal
- 种子表达式使用 v2 Expression 的解析器 (parse_expression)
"""
...
from .validator import run_backtest_from_signal
