...
对每个组合执行回测 + Walk-Forward验证，汇总排序输出。

用法:
    from signals.v2 import FeatureSpace, GridSearch, PRESETS

    fs = FeatureSpace().fit(data)

    # 方式1: 从preset模板
    searcher = GridSearch(fs, data)
    searcher.from_preset('atr', param_grid={'?': [7, 10, 14, 20]})
    results = searcher.run()

    # 方式2: 自定义表达式
    searcher = GridSearch(fs, data)
    searcher.from_template("thr_mean(ATR(?))", param_grid={'?': [7, 10, 14]})
    results = searcher.run()

    # 方式3: 多参数展开
    searcher.from_template("ATR(?) & TRIMA(?)", param_grid={'?': [[7, 40], [7, 60], [14, 60]]})
"""
