"""
expression_factor.py — 表达式因子适配器 + 批量探索器
=============================================================================

将 FactorExpression 表达式字符串包装为 Factor 子类，使其可接入
grid_search、pipeline、validator 等所有 v2 基础设施。

同时提供 AlphaExplorer 批量探索器，支持任意因子公式集合的
表达式求值 → IC 检验 → Pipeline 回测 → 汇总排序。

与 alpha101/alpha191 的关系：
  - 本模块是通用工具层，不包含因子公式
  - alpha101.py / alpha191.py 提供公式字典
  - 通过 expression_factor() 工厂函数 + AlphaExplorer 组合使用

[新增] 2026-05-30  最初位于 alpha191.py
[重构] 2026-06-01  从 alpha191.py 拆分，职责归位 —— 本模块是工具，不是公式数据
=============================================================================
"""
...
def expression_factor(alpha_id: str, formulas: Dict[str, str] = None,
                      **kwargs) -> ExpressionFactor:
    """根据 alpha ID 和公式字典创建 ExpressionFactor 实例

    Args:
        alpha_id: 因子 ID，如 'alpha001'、'gtja_001'
        formulas: 公式字典，默认使用 alpha101 的 ALPHA101
    ...
    """
    if formulas is None:
        from .alpha101 import ALPHA101 as formulas  # noqa: reimport for default
...
class AlphaExplorer:
    ...
    不绑定特定公式源（alpha101 / alpha191 / 自定义均可）。

    Example:
        >>> from factor.v2.alpha101 import ALPHA101
    ...
    def __init__(self,
        ...
        Args:
            ...
            formulas: 公式字典 {id: expression_str}，默认使用 ALPHA101
            cost_rate: 交易成本费率
            freq: 调仓频率 ('ME', 'W', '5D' 等)
            top_n: 持仓数
        """
        ...
        if formulas is None:
            from .alpha101 import ALPHA101 as formulas  # noqa
    ...
    def run(self,
        ...
        Args:
            alpha_ids: 因子 ID 列表，None 表示全部已定义公式
            verbose: 是否打印进度
