"""
factor/v4/cs_resolver.py — 截面函数 AST 解析器 (注册表驱动 + 单遍 bottom-up)

核心设计:
  1. 自动发现: 从 signals/v4 FUNC_REGISTRY 中以 cs_ 前缀自动识别截面函数
...
[新增] 2026-06-22 从 expression.py 提取, 替代 while-loop 多轮迭代
"""
...
import signals.v4.registry as registry

# ============================================================
# 截面函数注册表 — 自动从 signals/v4 发现所有 cs_* 函数
# ============================================================

CROSS_SECTIONAL_FUNCTIONS: Set[str] = {
    name for name in registry.FUNC_REGISTRY
...
class CsResolver:
    ...
    def _resolve_node(self, node: ast.Call, data: Dict[str, np.ndarray]) -> ast.Name:
        ...
        if func_name == 'cs_rank':
            ...
            fn = registry.FUNC_REGISTRY.get(func_name)
            if fn is not None:
